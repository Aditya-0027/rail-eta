...
async function loadTrains(force=false){
 try{
  ...
  trains.forEach(t=>{if(t.delay>=10)delayed++;avg+=Number(t.confidence||0);
   tbody.innerHTML+=`<tr class="${selected===t.train_no?'selected-row':''}"><td><div class="train">${t.train_no} · ${t.name}</div><div class="route">${t.category||""} · ${t.zone||""}</div></td><td>${t.origin||"—"}<br>→ ${t.destination||"—"}</td><td>${badge(t.status)}</td><td><b>${t.speed!=null?t.speed:(t.segment_speed!=null?t.segment_speed:"—")}</b> ${t.speed!=null?"km/h":(t.segment_speed!=null?"km/h*":"")}</td><td><b>${(t.early_minutes||0)>0 ? `${t.early_minutes} min early` : `${t.delay ?? 0} min`}</b></td><td>${t.next_station||"—"}</td><td><b>${t.next_eta||"—"}</b></td><td>${t.confidence!=null?t.confidence+"%":"—"}</td><td><button class="view" onclick="selectTrain('${t.train_no}')">View</button></td></tr>`;
  });
...
async function selectTrain(no){
 selected=no;
 try{
  const [t,st,ev,env]=await Promise.all([api(`/api/trains/${no}`),api(`/api/trains/${no}/stations`),api(`/api/trains/${no}/events`),api(`/api/trains/${no}/environment`)]);
  ...
  const weatherText=w.condition?`${w.condition} · ${w.temperature_c??"—"}°C · rain ${w.rain_mm??0} mm · wind ${w.wind_kmh??"—"} km/h${t.weather_source==="next-station coordinates"?` · near ${t.weather_station||t.next_station||"next station"}`:""}`:(env.weather_error||t.weather_error||"Weather unavailable");
  const predictedEta=t.predicted_eta||t.next_eta||"—";
  const currentSpeed=t.speed!=null ? `${t.speed} km/h` : "Unavailable";
  const speedNote=t.speed_source==="stationary" ? "Train is halted at station" : (t.speed_source==="telemetry" ? "Live telemetry" : "Current telemetry unavailable");
  const segmentSpeed=t.segment_speed!=null ? `${t.segment_speed} km/h` : "Unavailable";
  $("#detailBody").innerHTML=`<div class="bigeta">${predictedEta}</div><div style="font-size:10px;color:#667085">Predicted next-station ETA · confidence ${pred.confidence??t.confidence??"—"}% · ${source}</div><div class="kv"><div><small>Current speed</small><b>${currentSpeed}</b><small>${speedNote}</small></div><div><small>Current delay</small><b>${(t.early_minutes||0)>0 ? `${t.early_minutes} min early` : `${t.delay ?? 0} min`}</b></div><div><small>Next station</small><b>${t.next_station||"—"}</b></div><div><small>Platform</small><b>${t.platform||"TBD"}</b></div><div><small>Position</small><b>${t.lat!=null&&t.lon!=null?Number(t.lat).toFixed(3)+", "+Number(t.lon).toFixed(3):"Not supplied"}</b></div><div><small>Updated</small><b>${formatUpdated(t.updated)}</b></div><div><small>Expected segment speed</small><b>${segmentSpeed}</b><small>Provider route estimate</small></div></div><div class="environment"><div><b>🌦 Weather</b><span>${weatherText}</span></div><div><b>🚧 Congestion</b><span>${cong.level||"—"} · score ${cong.score??"—"}/100</span></div><div><b>🚦 Signal</b><span>${sig.status||"NOT SUPPLIED"}</span></div><div><b>🤖 ETA model</b><span>+${pred.predicted_extra_delay??0} min adjustment · ${pred.model||"prediction layer"}</span></div></div><button class="btn" onclick="refreshSelected('${t.train_no}')">⚡ Refresh live data</button><div class="section"><h3>Upcoming ETA Forecast</h3>${stations}</div><div class="section"><h3>Delay / Event Feed</h3>${events}</div>`;
