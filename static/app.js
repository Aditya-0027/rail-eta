...
async function loadTrains(force=false){
 try{
  ...
  trains.forEach(t=>{if(t.delay>=10)delayed++;avg+=Number(t.confidence||0);
   tbody.innerHTML+=`<tr class="${selected===t.train_no?'selected-row':''}"><td><div class="train">${t.train_no} · ${t.name}</div><div class="route">${t.category||""} · ${t.zone||""}</div></td><td>${t.origin||"—"}<br>→ ${t.destination||"—"}</td><td>${badge(t.status)}</td><td><b>${t.speed ?? "—"}</b> km/h</td><td><b>${t.delay ?? 0} min</b></td><td>${t.next_station||"—"}</td><td><b>${t.next_eta||"—"}</b></td><td>${t.confidence!=null?t.confidence+"%":"—"}</td><td><button class="view" onclick="selectTrain('${t.train_no}')">View</button></td></tr>`;
  });
