...
async function selectTrain(no){
 ...
 try{
  ...
  $("#detailBody").innerHTML=`<div class="bigeta">${predictedEta}</div><div style="font-size:10px;color:#667085">Predicted next-station ETA · confidence ${pred.confidence??t.confidence??"—"}% · ${source}</div><div class="kv"><div><small>${t.speed_source==="segment_estimate"?"Segment speed estimate":"Current speed"}</small><b>${t.speed!=null?t.speed:(t.segment_speed!=null?t.segment_speed:"Unavailable")}${(t.speed!=null||t.segment_speed!=null)?" km/h":""}</b></div><div><small>Current delay</small><b>${(t.early_minutes||0)>0 ? `${t.early_minutes} min early` : `${t.delay ?? 0} min`}</b></div><div><small>Next station</small><b>${t.next_station||"—"}</b></div><div><small>Platform</small><b>${t.platform||"TBD"}</b></div><div><small>Position</small><b>${t.lat!=null&&t.lon!=null?Number(t.lat).toFixed(3)+", "+Number(t.lon).toFixed(3):"Not supplied"}</b></div><div><small>Updated</small><b>${formatUpdated(t.updated)}</b></div></div><div class="environment"><div><b>🌦 Weather</b><span>${weatherText}</span></div><div><b>🚧 Congestion</b><span>${cong.level||"—"} · score ${cong.score??"—"}/100</span></div><div><b>🚦 Signal</b><span>${sig.status||"NOT SUPPLIED"}</span></div><div><b>🤖 ETA model</b><span>+${pred.predicted_extra_delay??0} min adjustment · ${pred.model||"prediction layer"}</span></div></div><button class="btn" onclick="refreshSelected('${t.train_no}')">⚡ Refresh live data</button><div class="section"><h3>Upcoming ETA Forecast</h3>${stations}</div><div class="section"><h3>Delay / Event Feed</h3>${events}</div>`;
